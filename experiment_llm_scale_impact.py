...
输出：
- results/llm-scale-impact-results.xlsx
...
def load_network_data(num_llms: int,
                      user_dist: str,
                      llm_dist: str,
                      sheets_root='sheets'):
    """
...
def run_llm_scale_experiments(sheets_root='sheets', results_root='results'):
    """
    运行LLM规模影响实验
    遍历所有LLM数量和分布组合，结果按"user分布-llm分布"分sheet存储
    """
    print("=" * 80)
    print("LLM规模影响实验（所有分布组合）")
    print("=" * 80)
    print(f"实验配置：")
    ...
    # 输出文件路径
    output_path = os.path.join(results_root, 'llm-scale-impact-results.xlsx')
    ...
    # 统计信息
    total_combinations = len(LLM_COUNTS) * len(DISTRIBUTION_TYPES) * len(
        DISTRIBUTION_TYPES)
        ...
        for user_dist in DISTRIBUTION_TYPES:
            for llm_dist in DISTRIBUTION_TYPES:
                ...
                print(f"[{current_combination}/{total_combinations}] "
                      f"LLM数={num_llms}, user={user_dist}, llm={llm_dist}")
                print(f"{'='*80}")

                # 加载网络数据
                try:
                    network, users, llms, user_ideal_llms = load_network_data(
                        num_llms, user_dist, llm_dist, sheets_root)
                except Exception as e:
