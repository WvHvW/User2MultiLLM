...
2. no-split（带汇集图）
   ...
   2. 按当前最大用户节点的流量作为推流量，寻找S -> T的最短路径，根据实际路径选择的用户推送流量
   3. 当前无法找到路径时，踢出这个用户，使用第二大用户流量作为推流量推送。
3. bottleneck-split
...
5. taskoffloading
   ...
   2. 用户分配顺序按默认索引顺序即可
6. taskoffloading（带汇集图）
   1. 所有用户连接到超源S，所有LLM连接到超汇T
   2. 按当前最大用户节点的流量作为推流量，寻找S -> T的最短路径，根据实际路径选择的用户推送流量
   3. 推送流量的大小为用户剩余流量和llm剩余容量的最小值
7. bottleneck-split-augment
   1. 首先生成汇集图：将所有用户连接到超级源点S，S->user的链路的属性满足以下要求
      1. S->user的边是单向物理边，生成残量网络时没有反向边
      2. 链路带宽为user.bw
      3. cost为极小值。
